...
const fetchCountry = (state, action) => {
  const country = state.countries.find(
    (c) => c.alpha3Code === action.alpha3Code
  );
  return updateObject(state, {
...
const filterRegion = (state, action) => {
  ...
  if (state.searchTerm) {
    regionCountries = state.countries.filter(
      (c) =>
        c.region.toLowerCase() === filterRegion.toLowerCase() &&
        c.name.toLowerCase().includes(state.searchTerm)
    );
  } else {
    regionCountries = state.countries.filter(
      (c) => c.region.toLowerCase() === filterRegion.toLowerCase()
    );
  }
  return updateObject(state, {
    filteredCountries: regionCountries,
...
const searchCountrySuccess = (state, action) => {
  ...
  if (!searchTerm) {
    ...
    if (regionName === REGIONS[0]) {
      searchCountriesResult = state.countries.filter((c) =>
